 ...
 *  USA
 *
 *  $Id: ircdauth.c,v 1.2 2003/03/10 06:09:05 demond Exp $
 */
...

{
  ...
  sendto_one(client, form_str(client,RPL_WELCOME),
	     me.name,
	     client->name,
	     client->name);

  /* This is a duplicate of the NOTICE but see below...*/
  sendto_one(client, form_str(client,RPL_YOURHOST),
	     me.name,
	     client->name,
  ...
  */
  ...
  sendto_one(client, form_str(client,RPL_CREATED),
	     me.name,
	     client->name,
	     creation);

  sendto_one(client, form_str(client,RPL_MYINFO),
	     me.name,
	     client->name,
	     me.name,
	     version);
  ...
  if(ConfigFileEntry.short_motd) {
    ...
    sendto_one(client, form_str(client,RPL_MOTDSTART),
	       me.name,
	       client->name,
	       me.name);

    sendto_one(client,
	       form_str(client,RPL_MOTD),
	       me.name,
	       client->name,
	       "*** This is the short motd ***");
    
    sendto_one(client, form_str(client,RPL_ENDOFMOTD),
	       me.name,
	       client->name);
