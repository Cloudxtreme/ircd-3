 ...
 *  USA
 *
 *  $Id: channel_mode.c,v 1.3 2003/03/13 16:34:05 demond Exp $
 */
...
extern void clear_pace_queues(struct Channel *);
...
static void chm_subst(struct Client *, struct Client *, struct Channel *,
                      int, int *, char **, int *, int, int, char, void *,
                      const char *chname);

static void chm_deny(struct Client *, struct Client *, struct Channel *,
                     int, int *, char **, int *, int, int, char, void *,
                     const char *chname);
...
static int channel_capabs[] = { CAP_HOPS, CAP_AOPS, CAP_EX, 
								CAP_IE, CAP_SW, CAP_DENY, CAP_UID };
...
add_id(struct Client *client_p, struct Channel *chptr, char *banid, int type)
{
  ...
  if ((!IsServer(client_p)) && (chptr->num_mask >= ConfigChannel.max_bans))
  {
    ...
    {
      sendto_one(client_p, form_str(client_p,ERR_BANLISTFULL),
              ...
              char *mbuf, char *pbuf)
{
  ...
  *pbuf = '\0';
  ...
  if (chptr->mode.mode & MODE_REGCHAN)
    *mbuf++ = 'r';
  if (chptr->mode.mode & MODE_PACE)
    *mbuf++ = 'x';
  if (chptr->mode.mode & MODE_NOREPEAT)
    *mbuf++ = 'z';
  if (chptr->mode.mode & MODE_REGNICKS)
    *mbuf++ = 'R';
  if (chptr->mode.mode & MODE_REGSPEAK)
    *mbuf++ = 'M';
  if (chptr->mode.mode & MODE_CYRNICKS)
    *mbuf++ = 'C';
...
#define SM_ERR_NOTSERVICES      0x00000400      /* not from services client */
#define SM_ERR_CANTDEOPSERV		0x00000800		/* can't deop service bots */
#define SM_ERR_RPL_W			0x00001000
           ...
           const char *chname)
{
  ...
  *errors |= SM_ERR_UNKNOWN;
  sendto_one(source_p, form_str(source_p,ERR_UNKNOWNMODE), me.name, source_p->name, c);
}
           ...
           const char *chname)
{
  ...
  if (mode_type & MODE_REGCHAN)
  {
    if (!(IsServer(source_p) || find_z_conf((char *)source_p->user->server)))
    {
      if (MyClient(source_p) && !(*errors & SM_ERR_NOTSERVICES))
		sendto_one(source_p,
			":%s NOTICE %s :*** NOTICE -- Chanmode +r can only be set by services",
			me.name, source_p->name);
	  *errors |= SM_ERR_NOTSERVICES;
      return;
	}
  } 
  else
  ...
  {
    /* setting - */
    if (mode_type & MODE_PACE)
    	clear_pace_queues(chptr);
  ...
  if (dir == 0 || parc <= *parn)
  {
    ...
    *errors |= SM_ERR_RPL_B;
    ...
    if ((chptr->mode.mode & MODE_HIDEOPS) && (alev < CHACCESS_HALFOP))
      ...
      {
        banptr = ptr->data;
        sendto_one(client_p, form_str(client_p,RPL_BANLIST),
    ...
    sendto_one(source_p, form_str(source_p,RPL_ENDOFBANLIST), me.name,
               source_p->name, chname);
    return;
  }
           ...
           const char *chname)
{
  ...
  if ((dir == MODE_QUERY) || parc <= *parn)
  {
    ...
    *errors |= SM_ERR_RPL_E;
    ...
    {
      banptr = ptr->data;
      sendto_one(client_p, form_str(client_p,RPL_EXCEPTLIST),
                 me.name, client_p->name, chname,
                 banptr->banstr, banptr->who, banptr->when);
    }
    sendto_one(source_p, form_str(source_p,RPL_ENDOFEXCEPTLIST), me.name,
               source_p->name, chname);
    return;
  }
          ...
          const char *chname)
{
  ...
  if ((dir == MODE_QUERY) || parc <= *parn)
  {
    ...
    *errors |= SM_ERR_RPL_I;
    ...
    {
      banptr = ptr->data;
      sendto_one(client_p, form_str(client_p,RPL_INVITELIST), me.name,
                 client_p->name, chname, banptr->banstr,
                 banptr->who, banptr->when);
    }
    sendto_one(source_p, form_str(source_p,RPL_ENDOFINVITELIST), me.name,
               source_p->name, chname);
    return;
  }
...
static void
chm_subst(struct Client *client_p, struct Client *source_p,
          struct Channel *chptr, int parc, int *parn,
          char **parv, int *errors, int alev, int dir, char c, void *d,
          const char *chname)
{
  ...
  if(!ConfigChannel.use_subst && MyClient(source_p) && 
    (dir == MODE_ADD) && (parc > *parn))
  {
    if((*errors & SM_ERR_RPL_W) != 0)
      return;
  ...
  if ((dir == MODE_QUERY) || parc <= *parn)
  {
    ...
    *errors |= SM_ERR_RPL_W;
    ...
    DLINK_FOREACH(ptr, chptr->substlist.head)
    {
      banptr = ptr->data;
      sendto_one(client_p, form_str(client_p,RPL_SUBSTLIST), me.name,
                 client_p->name, chname, banptr->banstr,
                 banptr->who, banptr->when);
    }
    sendto_one(source_p, form_str(source_p,RPL_ENDOFSUBSTLIST), me.name,
               source_p->name, chname);
    return;
  }
  ...
  /*  mask = pretty_mask(raw_mask);		messes up subst expression */
    mask = raw_mask;
  ...
  if(dir == MODE_ADD)
  {
    if((add_id(source_p, chptr, mask, CHFL_SUBST) == 0) && MyClient(source_p))
      return;

    mode_changes[mode_count].letter = c;
    mode_changes[mode_count].dir = MODE_ADD;
    mode_changes[mode_count].caps = CAP_SW;
    mode_changes[mode_count].nocaps = 0;
    
    if(ConfigChannel.use_subst)
      mode_changes[mode_count].mems = ONLY_CHANOPS_HALFOPS;
    else
      mode_changes[mode_count].mems = ONLY_SERVERS;
  ...
  {
    if (del_id(chptr, mask, CHFL_SUBST) == 0)
    {
      /* mask isn't a valid ban, check raw_mask */
      if((del_id(chptr, raw_mask, CHFL_SUBST) == 0) && MyClient(source_p))
      {
        /* nope */
        return;
      }
...
static void
chm_deny(struct Client *client_p, struct Client *source_p,
          struct Channel *chptr, int parc, int *parn,
          char **parv, int *errors, int alev, int dir, char c, void *d,
          const char *chname)
{
  ...
  if(!ConfigChannel.use_denies && MyClient(source_p) && 
    (dir == MODE_ADD) && (parc > *parn))
  {
    if((*errors & SM_ERR_RPL_D) != 0)
      return;
  ...
  if ((dir == MODE_QUERY) || parc <= *parn)
  {
    ...
    *errors |= SM_ERR_RPL_D;
    ...
    DLINK_FOREACH(ptr, chptr->denylist.head)
    {
      banptr = ptr->data;
      sendto_one(client_p, form_str(client_p,RPL_DENYLIST), me.name,
                 client_p->name, chname, banptr->banstr,
                 banptr->who, banptr->when);
    }
    sendto_one(source_p, form_str(source_p,RPL_ENDOFDENYLIST), me.name,
               source_p->name, chname);
    return;
  }
  ...
  /*  mask = pretty_mask(raw_mask);		messes up gecos deny expression */
    mask = raw_mask;
  ...
  if(dir == MODE_ADD)
  {
    if((add_id(source_p, chptr, mask, CHFL_DENY) == 0) && MyClient(source_p))
      return;

    mode_changes[mode_count].letter = c;
    mode_changes[mode_count].dir = MODE_ADD;
    mode_changes[mode_count].caps = CAP_DENY;
    mode_changes[mode_count].nocaps = 0;
    
    if(ConfigChannel.use_denies)
      mode_changes[mode_count].mems = ONLY_CHANOPS_HALFOPS;
    else
      mode_changes[mode_count].mems = ONLY_SERVERS;
  ...
  {
    if (del_id(chptr, mask, CHFL_DENY) == 0)
    {
      /* mask isn't a valid ban, check raw_mask */
      if((del_id(chptr, raw_mask, CHFL_DENY) == 0) && MyClient(source_p))
      {
        /* nope */
        return;
      }
       ...
       const char *chname)
{
  ...
  if (!IsMember(targ_p, chptr))
  {
    if (!(*errors & SM_ERR_NOTONCHANNEL))
      sendto_one(source_p, form_str(source_p,ERR_USERNOTINCHANNEL), me.name,
  ...
  if (find_z_conf((char *)targ_p->user->server) && !IsOper(source_p)
	&& (dir == MODE_DEL) && (c == 'o') && t_op) 
  {
	if (MyClient(source_p) && !(*errors & SM_ERR_CANTDEOPSERV))
	  sendto_one(source_p, form_str(source_p,ERR_CANTDEOPSERVICES),
			me.name, source_p->name, targ_p->name, chname);
	*errors |= SM_ERR_CANTDEOPSERV;
	return;
  }
...
static struct ChannelMode ModeTable[255] =
{
  ...
  {chm_nosuch, NULL},                             /* A */
  {chm_nosuch, NULL},                             /* B */
  {chm_simple, (void *) MODE_CYRNICKS},           /* C */
  {chm_nosuch, NULL},                             /* D */
  {chm_nosuch, NULL},                             /* E */
  {chm_nosuch, NULL},                             /* F */
  {chm_nosuch, NULL},                             /* G */
  ...
  {chm_nosuch, NULL},                             /* K */
  {chm_nosuch, NULL},                             /* L */
  {chm_simple, (void *) MODE_REGSPEAK},           /* M */
  {chm_nosuch, NULL},                             /* N */
  {chm_nosuch, NULL},                             /* O */
  {chm_nosuch, NULL},                             /* P */
  {chm_nosuch, NULL},                             /* Q */
  {chm_simple, (void *) MODE_REGNICKS},           /* R */
  ...
  {chm_subst, NULL},                              /* w */
#ifdef USEPACE
  {chm_simple, (void *) MODE_PACE},               /* x */
#else
  {chm_nosuch, NULL},				              /* x */
#endif
  {chm_nosuch, NULL},                             /* y */
  {chm_simple, (void *) MODE_NOREPEAT},           /* z */
};
/* *INDENT-ON* */
                  ...
                  struct Channel *chptr, char *chname)
{
   ...
   */
  /*
   * nah, don't send server bursting +e/+I/+d/+w to non-ops
   */   
   for (i=0; i < mode_count; i++) {
     char c = mode_changes[i].letter;
     if (c!='e' && c!='I' && c!='d' && c!='w') {
     	regular = 1;
     	break;
     }
...
#ifdef ANONOPS
  st = (chptr->mode.mode & MODE_HIDEOPS) ? ONLY_CHANOPS_HALFOPS : 
  	((regular)? ALL_MEMBERS : (IsServer(source_p)? ONLY_CHANOPS_HALFOPS : ALL_MEMBERS));
#else
  st = (regular)? ALL_MEMBERS : (IsServer(source_p)? ONLY_CHANOPS_HALFOPS : ALL_MEMBERS);
#endif
