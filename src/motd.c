 ...
 *  USA
 *
 *  $Id: motd.c,v 1.2 2003/03/10 06:09:05 demond Exp $
 */
  ...
  switch(motdType)
    ...
    case USER_MOTD:
      ...
      if (motdToPrint->contentsOfFile == (MessageFileLine *)NULL)
        {
          sendto_one(source_p, form_str(source_p,ERR_NOMOTD), me.name, nick);
          return 0;
        }

      sendto_one(source_p, form_str(source_p,RPL_MOTDSTART), me.name, nick, me.name);
      ...
      sendto_one(source_p, form_str(source_p,RPL_ENDOFMOTD), me.name, nick);
      return 0;
      /* NOT REACHED */
      break;
