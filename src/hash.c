 ...
 *  USA
 *
 *  $Id: hash.c,v 1.4 2003/03/10 06:09:04 demond Exp $
 */
...
/*
 * services stuff...	-demond
 */
void clear_svs_client_hash_table()
{
  int i;
  struct Client* tmp;

  for (i=0; i < U_MAX; i++)
    if (tmp = (struct Client*)clientTable[i].list)
      for (; tmp; tmp = tmp->hnext) {
      	if (tmp->svsflags & FLAGS_SVS_IDENT)
...
void do_local_clients(void (*func)(), char *mask, void *data)
{
  int i;
  struct Client* tmp;
  ...
  for (i=0; i < U_MAX; i++)
    if (tmp = (struct Client*)clientTable[i].list)
      for (; tmp; tmp = tmp->hnext)
		if (MyClient(tmp) && match(mask, tmp->name))
			func(tmp, data);		
}

void do_channel_local(void (*func)(), char *chan, void *data)
{
	int i, num = 0;
	dlink_node *ptr;
	dlink_list *lists[10];
	...
	lists[num++] = &chptr->locpeons;
	lists[num++] = &chptr->locvoiced;
#ifdef HALFOPS
	lists[num++] = &chptr->lochalfops;
#endif
#ifdef REQUIRE_OANDV
	lists[num++] = &chptr->locchanops_voiced;
#endif
	lists[num++] = &chptr->locchanops;

	for (i=0; i < num; i++)
		DLINK_FOREACH(ptr, lists[i]->head)
			func(ptr->data, data);
}

int services_present()
{
  int i;
  struct Client* tmp;
  ...
  for (i=0; i < U_MAX; i++)
    if (tmp = (struct Client*)clientTable[i].list)
      for (; tmp; tmp = tmp->hnext)
		if (IsServer(tmp))
          if (find_z_conf(tmp->name))
            return 1;
  
  return 0;
}

struct Client* find_services()
{
  unsigned int hashv;
  struct ConfItem *aconf;
  struct Client *tmp;

  extern struct ConfItem *z_conf;
  for (aconf = z_conf; aconf; aconf = aconf->next)
    {
      if (BadPtr(aconf->name))
		continue;

      hashv = hash_nick_name(aconf->name);
      ...
      for (; tmp; tmp = tmp->hnext)
		{
	  	  if (!IsServer(tmp)) 
	        continue;
	  	  if (irccmp(aconf->name, tmp->name) == 0)
	        return tmp;
		}
    }
...
int   add_to_watch_hash_table(char *nick, aClient *cptr)
{
    ...
    {
	lp = anptr->watch;
	anptr->watch = (Link *)MyMalloc(sizeof(Link));
...
int hash_check_watch(aClient *cptr, int reply)
{
    ...
    if ((anptr = (aWatch *)watchTable[hashv]))
	while (anptr && irccmp(anptr->nick, cptr->name))
	    anptr = anptr->hnext;
    if (!anptr)
	...
	sendto_one(lp->value.cptr, form_str(lp->value.cptr,reply), me.name,
...
int   del_from_watch_hash_table(char *nick, aClient *cptr)
{
    ...
    if (!lp)
	sendto_realops_flags(FLAGS_DEBUG, L_ALL,
	       "WATCH debug error: del_from_watch_hash_table "
...
int   hash_del_watch_list(aClient *cptr)
{
	...
	if (!lp)
	    sendto_realops_flags(FLAGS_DEBUG, L_ALL,
	           "WATCH Debug error: hash_del_watch_list "
