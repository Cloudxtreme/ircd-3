 ...
 *  USA
 *
 *  $Id: s_user.c,v 1.7 2003/03/10 06:09:06 demond Exp $
 */
...
show_lusers(struct Client *source_p) 
{
  if(!ConfigServerHide.hide_servers || IsOper(source_p))
    sendto_one(source_p, form_str(source_p,RPL_LUSERCLIENT), me.name, source_p->name,
              (Count.total-Count.invisi), Count.invisi, Count.server);
  else
    sendto_one(source_p, form_str(source_p,RPL_LUSERCLIENT), me.name, source_p->name,
              (Count.total-Count.invisi), Count.invisi, 1);
  if (Count.oper > 0)
    sendto_one(source_p, form_str(source_p,RPL_LUSEROP), me.name, source_p->name,
               Count.oper);

  if (Count.unknown > 0)
    sendto_one(source_p, form_str(source_p,RPL_LUSERUNKNOWN), me.name, source_p->name,
               Count.unknown);

  if (Count.chan > 0)
    sendto_one(source_p, form_str(source_p,RPL_LUSERCHANNELS),
               me.name, source_p->name, Count.chan);

  if(!ConfigServerHide.hide_servers || IsOper(source_p))
    {
      sendto_one(source_p, form_str(source_p,RPL_LUSERME),
                 me.name, source_p->name, Count.local, Count.myserver);
      sendto_one(source_p, form_str(source_p,RPL_LOCALUSERS), me.name, source_p->name,
  ...
  else
    {
      ...
      sendto_one(source_p, form_str(source_p,RPL_LOCALUSERS), 
                 me.name, source_p->name, Count.total, Count.max_tot);
    }

  sendto_one(source_p, form_str(source_p,RPL_GLOBALUSERS), me.name, source_p->name,
             Count.total, Count.max_tot);

  sendto_one(source_p, form_str(source_p,RPL_IDUSERS), me.name, source_p->name,
             Count.identified, Count.max_identified);

  if(!ConfigServerHide.hide_servers || IsOper(source_p))
    sendto_one(source_p, form_str(source_p,RPL_STATSCONN), me.name, source_p->name,
...
show_isupport(struct Client *source_p) 
{
  ...
  sendto_one(source_p, form_str(source_p,RPL_ISUPPORT), me.name, source_p->name, 
  	     isupportbuffer);
			...
			char *nick, char *username)
{
  ...
  strcpy(source_p->ip, source_p->localClient->sockhost);
  ...
  if (!BadPtr(aconf->passwd) &&
    ...
    {
      ServerStats->is_ref++;
      sendto_one(source_p, form_str(source_p,ERR_PASSWDMISMATCH),
		 me.name, source_p->name);
  ...
  if (source_p->svsflags & FLAGS_SVS_IDENT)
...
#define NETID "bnet"
  ...
  if (!IsConfDoSpoofIp(aconf))
    strcpy(source_p->realhost, source_p->host);

  /* mask the appropriate client host portion:
   * - leftmost part if resolving host 
   *   (but not if just one dot in the host)
   * - rightmost part if numeric IP
   *                                   -demond
   */
#ifdef IPV6
  if (source_p->localClient->aftype != AF_INET6)
#endif   
  if (IsConfMaskHost(aconf))
    {
      int alpha = 0, dots = 0;
      char buf[HOSTLEN + 1];
      char *p = source_p->host;
      static char *get_mask(char *, int);
      ...
      if (alpha && (dots > 1)) 
		{
	  		...
	  		if (strlen(p) + MAXNETID + 6 <= HOSTLEN)
	    	{
	      		strcpy(source_p->host, get_mask(buf, 0));
	      		strcat(source_p->host, p);
	    	}
		} 
      else if (!alpha) 
		{
	  		...
	  		if (strlen(buf) + 7 <= HOSTLEN)
	    	{
	      		strcpy(source_p->host, buf);
	      		strcat(source_p->host, get_mask(NULL, atoi(p)));
	    	}
		}
    }
...
static char *get_mask(char *s, int n)
{
  static char buf[MAXNETID + 6];
  char *p = buf;
  char *netid = ConfigFileEntry.netid[0]? ConfigFileEntry.netid : "bnet";
  ...
  if (s) 
    {
      ...
      while (*s)
		{
	  		h = (h << 4) - (h + (unsigned char)ToLower(*s++));
		}
			 ...
			 char *nick, char *username)
{
  ...
  if (client_p->eob && strcmp(source_p->ip, "<burst>") && !IsRelay(source_p->servptr))
	  sendto_realops_flags(FLAGS_REMOTE, L_ALL,
	  		"Remote client: %s (%s@%s) [%s] {%s}",
	  		nick, username, source_p->realhost, source_p->ip, source_p->user->server);
		 ...
		 struct User *user, char *nick)
{
  ...
  if (!ServerInfo.hub && uplink && IsCapable(uplink,CAP_LL)
      && client_p != uplink) 
    {
      if (IsCapable(uplink, CAP_SVS))
        svshopcount = source_p->hopcount + 1 + (source_p->svsflags << 8);
      else 
        svshopcount = source_p->hopcount + 1;

      if (IsCapable(uplink, CAP_UID) && HasID(source_p))
		{
          sendto_one(uplink, "CLIENT %s %d %lu %s %s %s %s %s %s %s :%s",
                     nick, svshopcount, source_p->tsinfo,
                     ubuf, source_p->username, source_p->host, user->server,
                     user->id, IsCapable(uplink, CAP_HOSTMASK)?
                     source_p->realhost : "", IsCapable(uplink, CAP_IP)?
                     source_p->ip : "", source_p->info);
		}
      else
		{
          sendto_one(uplink, "NICK %s %d %lu %s %s %s %s %s %s :%s",
                     nick, svshopcount, source_p->tsinfo,
                     ubuf, source_p->username, source_p->host, user->server,
                     IsCapable(uplink, CAP_HOSTMASK)? source_p->realhost : "",
                     IsCapable(uplink, CAP_IP)? source_p->ip : "",
                     source_p->info);
		}
    }
  else
      ...
      for (server_node = serv_list.head; server_node; server_node = server_node->next)
		{
          ...
          if (IsCapable(server, CAP_SVS))
            ...
            sendto_one(server, "CLIENT %s %d %lu %s %s %s %s %s %s %s :%s",
                       nick, svshopcount, source_p->tsinfo,
                       ubuf, source_p->username, source_p->host, user->server,
                       user->id, IsCapable(server, CAP_HOSTMASK)?
                       source_p->realhost : "", IsCapable(server, CAP_IP)?
                       source_p->ip : "", source_p->info);
	  	  else
            sendto_one(server, "NICK %s %d %lu %s %s %s %s %s %s :%s",
                       nick, svshopcount, source_p->tsinfo,
                       ubuf, source_p->username, source_p->host, user->server,
                       IsCapable(server, CAP_HOSTMASK)? source_p->realhost : "",
                       IsCapable(server, CAP_IP)? source_p->ip : "",
                       source_p->info);
		}
    }
...
{
  struct User* user;
  char *hend = host + strlen(host) - 1;
  char *send = server + strlen(server) - 1;
  ...
  /* mIRC kludge here */
  if (*host=='"' && *hend=='"' && *server=='"' && *send=='"')
	  user->known = 1;
  else
	  user->known = IsUserPar(host, server);

  if (!IsUnknown(source_p))
    {
      sendto_one(source_p, form_str(source_p,ERR_ALREADYREGISTRED), me.name, nick);
      return 0;
    }
		   ...
		   char *realhost, char *ip, char *id)
{
  ...
  if (realhost)
    strcpy(source_p->realhost, realhost);
  else
    strcpy(source_p->realhost, "<none>");

  if (client_p->eob) {
	if (ip)
		strcpy(source_p->ip, ip);
	else
		strcpy(source_p->ip, "255.255.255.255");
  } else
	  strcpy(source_p->ip, "<burst>");
...
user_mode(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
  ...
  if (source_p != target_p || target_p->from != source_p->from)
    {
       sendto_one(source_p, form_str(source_p,ERR_USERSDONTMATCH), me.name, parv[0]);
       return 0;
    }
  ...
  if (parc < 3)
    {
      ...
      *m = '\0';
      sendto_one(source_p, form_str(source_p,RPL_UMODEIS), me.name, parv[0], buf);
      return 0;
    }
  ...
  if(badflag)
    sendto_one(source_p, form_str(source_p,ERR_UMODEUNKNOWNFLAG), me.name, parv[0]);
...
{
  sendto_one(source_p, form_str(source_p,RPL_WELCOME), me.name, source_p->name, 
             ServerInfo.network_name, source_p->name );
  /* This is a duplicate of the NOTICE but see below...*/
  sendto_one(source_p, form_str(source_p,RPL_YOURHOST), me.name, source_p->name,
  ...
  */
  ...
  sendto_one(source_p, form_str(source_p,RPL_CREATED),me.name,source_p->name,creation);
  sendto_one(source_p, form_str(source_p,RPL_MYINFO), me.name, source_p->name,
  ...
  if (ConfigFileEntry.short_motd)
    {
      ...
      sendto_one(source_p, form_str(source_p,RPL_MOTDSTART),
		 me.name, source_p->name, me.name);
    
      sendto_one(source_p,
		 form_str(source_p,RPL_MOTD),
		 me.name, source_p->name,
		 "*** This is the short motd ***"
		 );

      sendto_one(source_p, form_str(source_p,RPL_ENDOFMOTD),
		 me.name, source_p->name);
    }
  else  
  ...
  if (IsRestricted(source_p))
    {
      sendto_one(source_p,form_str(source_p,ERR_RESTRICTED),
		 me.name, source_p->name);
    }
}
...
oper_up( struct Client *source_p, struct ConfItem *aconf )
{
  ...
  sendto_one(source_p, form_str(source_p,RPL_YOUREOPER), me.name, source_p->name);
