 ...
 *  USA
 *
 *  $Id: hostmask.c,v 1.2 2003/03/10 06:09:04 demond Exp $
 */
...
find_conf_by_address(const char *name, struct irc_inaddr *addr, int type,
                     int fam, const char *username, char *passwd)
{
    ...
    if (fam == AF_INET6)
    {
      for (b = 128; b >= 0; b -= 16)
      {
        for (arec = atable[hash_ipv6(addr, b)]; arec; arec = arec->next)
          ...
          {
			if (passwd && IsConfSkipOnPassFail(arec->aconf) 
			&& !BadPtr(arec->aconf->passwd) && strcmp(passwd, arec->aconf->passwd))
	  			continue;
...
find_address_conf(const char *host, const char *user,
                  struct irc_inaddr *ip, int aftype, char *passwd)
{
  struct ConfItem *iconf, *kconf;

  /* Find the best I-line... If none, return NULL -A1kmm */
  if (!(iconf = find_conf_by_address(host, ip, CONF_CLIENT, aftype, user, passwd)))
    return NULL;
...
find_dline_conf(struct irc_inaddr *addr, int aftype)
{
  ...
  if (eline)
    return eline;
  return find_conf_by_address(NULL, addr, CONF_DLINE | 1, aftype, 
  							  NULL, NULL);
}
  ...
  for (i = 0; i < ATABLE_SIZE; i++)
    for (arec = atable[i]; arec; arec = arec->next)
      if (arec->type == CONF_CLIENT)
      {
        ...
        sendto_one(client_p, form_str(client_p,RPL_STATSILINE), me.name,
  ...
  for (i = 0; i < ATABLE_SIZE; i++)
    for (arec = atable[i]; arec; arec = arec->next)
      if (arec->type == CONF_KILL)
      {
        ...
        get_printable_conf(aconf, &name, &host, &pass, &user, &port,
                           &classname);
        sendto_one(client_p, form_str(client_p,RPL_STATSKLINE), me.name,
