 ...
 *  USA
 *
 *  $Id: ircd.c,v 1.6 2003/05/24 00:07:46 demond Exp $
 */
...
char**  myargv;
int     dorehash   = 0;
int     debuglevel = DEBUG_DEBUG;/* Server debug level */
char*   debugmode  = "";        /*  -"-    -"-   -"-  */
  ...
  if (!getrlimit(RLIMIT_FD_MAX, &limit))
    {
      ...
      printf("Value of limit.rlim_max is %ld\n", (long)limit.rlim_max);
  ...
  if (!getrlimit(RLIMIT_CORE, &limit))
    {
      ...
      if (!setrlimit(RLIMIT_CORE, &limit))
	    printf("Value of corefile size is %lu\n", (unsigned long)limit.rlim_cur);
    }
#endif        /* RLIMIT_FD_MAX */
}
...
int main(int argc, char *argv[])
{
  ...
  /* well some ppl wanted low ports so...	-demond
  if(geteuid() == 0)
  {
  ...
  bindtextdomain("alt-language" , MSGPATH);
#endif
