 ...
 *  USA
 *
 *  $Id: ircd_parser.y,v 1.8 2003/05/24 00:07:46 demond Exp $
 */
...
%token	PARAM
%token  DUMMY
%token	DRONE_RATE_MAX
%token	DRONE_RATE_PERIOD
%token	DRONE_KLINE
%token	DRONE_BATCH
%token	DRONE_KLINE_TIME
%token	DREASON
%token	SPAM_WAIT
%token	REPEAT_GRACE
%token	SPAWN_PASS
...
serverinfo_item:        serverinfo_name | serverinfo_vhost |
                        ...
                        serverinfo_max_clients | serverinfo_gateway |
...
serverinfo_gateway: GATEWAY '=' TYES ';'
   {
     ServerInfo.gateway = 1;
   }
     |
     GATEWAY '=' TNO ';'
   {
     ServerInfo.gateway = 0;
   } ;
...
/***************************************************************************
 *  section equivalent, defines equivalent nicks          -demond
 ***************************************************************************/

equivalent_entry:   EQUIVALENT
  {
  }
  '{' equivalent_items '}' ';'
  {
...
equivalent_items:   equivalent_items equivalent_item |
                    equivalent_item
;
equivalent_item:    equivalent_nicks | equivalent_dummy | error
;
equivalent_nicks:   NICKS '=' QSTRING ';'
  {
    add_equiv_nicks(yylval.string);
  };

equivalent_dummy:   DUMMY '=' expr ';'
  {
  };

/***************************************************************************
 *  section relays, defines relay pseudo servers          -demond
 ***************************************************************************/
...
relays_items:       relays_items relays_item |
                    relays_item
;
relays_item:        relay_server | relay_dummy | error
;
relay_server:       NAME '=' QSTRING ';'
  {
    add_relay_server(yylval.string);
  };

relay_dummy:        DUMMY '=' expr ';'
  {
  };

/***************************************************************************
 *  section userpar, defines USER constant parameters 
 *  by various clients, mostly Java applets                    -demond
 ***************************************************************************/

userpar_entry:      USERPAR
  {
  }
  '{' userpar_items '}' ';'
  {
...
userpar_items:      userpar_items userpar_item |
                    userpar_item
;
userpar_item:       userpar_param | userpar_dummy | error
;
userpar_param:      PARAM '=' QSTRING ';'
  {
    add_userpar_param(yylval.string);
  };

userpar_dummy:      DUMMY '=' expr ';'
  {
  };
 
/***************************************************************************
 *  section clusters, defines foreign cluster gateways
 *  whose nicks we prepend with tokens                       -demond
 ***************************************************************************/

clusters_entry:      CLUSTERS
  {
  }
  '{' clusters_items '}' ';'
  {
...
clusters_items:      clusters_items clusters_item |
                     clusters_item
;
clusters_item:       cluster_bridge | cluster_dummy | error
;
cluster_bridge:      BRIDGE '=' QSTRING ';'
  {
    add_cluster_bridge(yylval.string);
  };

cluster_dummy:       DUMMY '=' expr ';'
  {
  };
...
auth_item:      auth_user | auth_passwd | auth_class |
                ...
                auth_maskhost | auth_skipfail | error;
...
auth_maskhost: MASKHOST '=' TYES ';'
  {
   yy_achead->flags |= CONF_FLAGS_MASKHOST;
  }
  |            MASKHOST '=' TNO ';'
  {
   yy_achead->flags &= CONF_FLAGS_MASKHOST;
...
auth_skipfail: SKIP_ON_FAIL '=' TYES ';'
  {
   yy_achead->flags |= CONF_FLAGS_SKIPFAIL;
  }
  |            SKIP_ON_FAIL '=' TNO ';'
  {
   yy_achead->flags &= CONF_FLAGS_SKIPFAIL;
...
/***************************************************************************
 *  section services, for defining which servers
 *  are to be treated specially (as services)       -demond 
 ***************************************************************************/
...
services_items:		services_items services_item |
			services_item
;
services_item:		services_name | services_user | services_host | error
;
services_name:		NAME '=' QSTRING ';'
  {
    MyFree(yy_aconf->name);
    DupString(yy_aconf->name, yylval.string);
  };

services_user:		USER '=' QSTRING ';'
  {
    MyFree(yy_aconf->user);
    DupString(yy_aconf->user, yylval.string);
  };

services_host:		HOST '=' QSTRING ';'
  {
    MyFree(yy_aconf->host);
    DupString(yy_aconf->host, yylval.string);
  };
...
general_item:       general_failed_oper_notice |
					...
					general_grace_time | general_spam_wait |
                    ...
                    general_idletime | general_spawn_pass |
                    ...
                    general_max_targets | general_repeat_grace |
                    ...
                    general_netid | general_cyrillic | general_totaltrans |
					general_drate_max | general_drate_period |
					general_drone_kline | general_drone_batch |
					general_drone_ktime | general_drone_reason |
                    error;

general_cyrillic: CYRILLIC '=' TNO ';'
  {
...
general_totaltrans: TOTALTRANS '=' TNO ';'
  {
...
general_grace_time:    GRACE_TIME '=' timespec ';'
  {
    ConfigFileEntry.grace_time = $3;
  } ;

general_spam_wait:     SPAM_WAIT '=' timespec ';'
  {
    ConfigFileEntry.spam_wait = $3;
  } ;

general_repeat_grace:  REPEAT_GRACE '=' timespec ';'
  {
    ConfigFileEntry.repeat_grace = $3;
  } ;

general_drate_max:     DRONE_RATE_MAX '=' NUMBER ';'
  {
    ConfigFileEntry.drone_rate_max = $3;
  } ;

general_drate_period:  DRONE_RATE_PERIOD '=' timespec ';'
  {
    ConfigFileEntry.drone_rate_period = $3;
  } ;

general_drone_ktime:   DRONE_KLINE_TIME '=' timespec ';'
  {
    ConfigFileEntry.drone_kline_time = $3;
  } ;

general_drone_kline:   DRONE_KLINE '=' TYES ';'
  {
...
general_drone_batch:   DRONE_BATCH '=' TYES ';'
  {
...
general_netid: NETID '=' QSTRING ';'
{
  strncpy(ConfigFileEntry.netid, yylval.string,
	      MAXNETID)[MAXNETID] = 0;
};

general_spawn_pass: SPAWN_PASS '=' QSTRING ';'
{
  strncpy(ConfigFileEntry.spawn_pass, yylval.string,
	      PASSWDLEN)[PASSWDLEN] = 0;
};

general_drone_reason: DREASON '=' QSTRING ';'
{
  strncpy(ConfigFileEntry.drone_reason, yylval.string,
	      MAXDREASON)[MAXDREASON] = 0;
};
...
general_message_locale: MESSAGE_LOCALE '=' QSTRING ';'
  {
    static char langenv[BUFSIZE];
    static char langenv2[BUFSIZE];
    if (strlen(yylval.string) > BUFSIZE-10)
      yylval.string[BUFSIZE-9] = 0;
    ircsprintf(langenv, "LANGUAGE=%s", yylval.string);
    putenv(langenv);
    ircsprintf(langenv2, "LC_MESSAGES=%s", yylval.string);
    putenv(langenv2);
#ifdef USE_GETTEXT
	{
...
umode_oitem:     T_BOTS
              ...
              | T_NCHANGE
  {
    ConfigFileEntry.oper_umodes |= FLAGS_NCHANGE;
              ...
              | T_REMOTE
  {
    ConfigFileEntry.oper_umodes |= FLAGS_REMOTE;
  }
              | T_REJ
  {
...
umode_item:	T_BOTS 
              ...
              | T_NCHANGE
  {
    ConfigFileEntry.oper_only_umodes |= FLAGS_NCHANGE;
              ...
              | T_REMOTE
  {
    ConfigFileEntry.oper_only_umodes |= FLAGS_REMOTE;
  }
              | T_REJ
  {
...
channel_use_subst:   USE_SUBST '=' TYES ';'
  {
    ConfigChannel.use_subst = 1;
  }
                        |
                     USE_SUBST '=' TNO ';'
  {
    ConfigChannel.use_subst = 0;
  } ;


channel_use_denies:   USE_DENIES '=' TYES ';'
  {
    ConfigChannel.use_denies = 1;
  }
                        |
                     USE_DENIES '=' TNO ';'
  {
    ConfigChannel.use_denies = 0;
  } ;
