 ...
 *  USA
 *
 *  $Id: send.c,v 1.4 2003/03/10 06:09:06 demond Exp $
 */
...
static int marknick(struct Client *client_p, char *buf);
                    ...
                    buf_head_t *linebuf)
{
  ...
  if (!MyClient(from) && IsPerson(to) && (to == from->from))
  {
    ...
    if (IsPerson(from))
      sendto_one(from, form_str(from,ERR_GHOSTEDCLIENT),
                     ...
                     buf_head_t *remote_linebuf, buf_head_t *uid_linebuf)
{
  ...
  buf_head_t my_winbuf, my_isobuf;
  int windone = 0, isodone = 0;
  buf_line_t *bl;
  
  linebuf_newbuf(&my_winbuf);
  linebuf_newbuf(&my_isobuf);
  bl = (buf_line_t *)local_linebuf->list.tail->data;
  linebuf_putmsg(&my_winbuf, NULL, NULL, "%s", bl->buf);
  linebuf_putmsg(&my_isobuf, NULL, NULL, "%s", bl->buf);
  ...
  DLINK_FOREACH_SAFE(ptr, ptr_next, list->head)
  {
    ...
    if (MyConnect(target_p) && IsRegisteredUser(target_p) && !IsDead(target_p))
    {
      ...
      {
      	/*buf_line_t *bl = (buf_line_t *)local_linebuf->list.tail->data;
	    int mark = marknick(target_p, bl->buf);
		
		if (mark) {
			bl->len++; local_linebuf->len++;
		}
					
		translate(target_p, bl->buf);*/
		if (IsSetTransWin(target_p)) {
		
			if (!windone) {
				bl = (buf_line_t *)my_winbuf.list.tail->data;
				translate(target_p, bl->buf);
				windone = 1;
			}
			send_linebuf(target_p, &my_winbuf);
			
		} else if (IsSetTransISO(target_p)) {
		
			if (!isodone) {
				bl = (buf_line_t *)my_isobuf.list.tail->data;
				translate(target_p, bl->buf);
				isodone = 1;
			}
			send_linebuf(target_p, &my_isobuf);
  ...
  linebuf_donebuf(&my_winbuf);
  linebuf_donebuf(&my_isobuf);
}
...
/*
 * add to channel NICK/QUIT pace queue
 */
void add_pace_send(struct Channel *chptr, char *sendbuf, int len)
{
  ...
  pace->next = NULL;
  pace->shownames = len;
  pace->text = MyMalloc(strlen(sendbuf) + 1);
  strcpy(pace->text, sendbuf);

  if (chptr->sendh)
    chptr->sendh->next = pace;
  chptr->sendh = pace;
  if (!chptr->sendt)
    chptr->sendt = pace;
...
void sendto_chanlists_local(struct Channel *chptr, char *sendbuf, int len)
{
  buf_head_t lb;
  linebuf_newbuf(&lb);
  linebuf_putmsg(&lb, NULL, NULL, "%s", sendbuf);

  sendto_list_local(&chptr->chanops, &lb);
  sendto_list_local(&chptr->halfops, &lb);
  sendto_list_local(&chptr->voiced, &lb);
  sendto_list_local(&chptr->peons, &lb);
...
sendto_common_channels_local(struct Client *user, const char *pattern, ...)
{
  ...
  if (user->user != NULL)
  {
    DLINK_FOREACH_SAFE(ptr, ptr_next, user->user->channel.head)
    {
	  ...
	  if (PaceChannel(chptr) && (chptr->sends < MAX_PACESEND))
	  	{
	  		buf_line_t *bl = (buf_line_t *)linebuf.list.tail->data;
	    	add_pace_send(chptr, bl->buf, bl->len);
	    }
	  else
	    {
...
sendto_list_local(dlink_list *list, buf_head_t *linebuf_ptr)
{
  ...
  linebuf_newbuf(&my_winbuf);
  linebuf_newbuf(&my_isobuf);
  bl = (buf_line_t *)linebuf_ptr->list.tail->data;
  linebuf_putmsg(&my_winbuf, NULL, NULL, "%s", bl->buf);
  linebuf_putmsg(&my_isobuf, NULL, NULL, "%s", bl->buf);
  ...
  DLINK_FOREACH_SAFE(ptr, ptr_next, list->head)
  {
	...
	if (ConfigFileEntry.totaltrans)
	  {
      	/*buf_line_t *bl = (buf_line_t *)linebuf_ptr->list.tail->data;
	    int mark = marknick(target_p, bl->buf);
		
		if (mark) {
			bl->len++; linebuf_ptr->len++;
		}
...
/*
 * this puts '?' in front of a translated cyrillic nick buffer
 *												-demond
 */
static int marknick(struct Client *client_p, char *buf)
{
  ...
  if (!ConfigFileEntry.cyrillic)
    return 0;

  if (!IsSetTransWin(client_p) && !IsSetTransISO(client_p))
    return 0;
