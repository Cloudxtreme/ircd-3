 ...
 *  USA
 *
 *  $Id: vchannel.c,v 1.2 2003/03/10 06:09:07 demond Exp $
 */
...
cjoin_channel(struct Channel *root, struct Client *source_p, char *name)
{
  ...
  if (on_sub_vchan(root, source_p))
    {
      sendto_one(source_p, form_str(source_p,ERR_ALREADYONVCHAN),
                 me.name, source_p->name, name);
      return NULL;
    }
  ...
  if (strlen(name) > CHANNELLEN - 15)
    {
      sendto_one(source_p, form_str(source_p,ERR_BADCHANNAME), me.name, source_p->name,
                 name);
      return NULL;
    }
...
show_vchans(struct Client *source_p, struct Channel *chptr, char *command)
{
  ...
  sendto_one(source_p, form_str(source_p,RPL_VCHANEXIST),
             me.name, source_p->name, chptr->chname, no_of_vchans);

  vchan_show_ids(source_p, chptr);

  sendto_one(source_p, form_str(source_p,RPL_VCHANHELP),
...
vchan_show_ids(struct Client *source_p, struct Channel *chptr)
{
  ...
  ircsprintf(buf, form_str(source_p,RPL_VCHANLIST), me.name, source_p->name,
             chptr->chname);
