 ...
 *
 *   $Id: m_operspy.c,v 1.2 2003/03/10 06:08:59 demond Exp $
 */
                       ...
                       int parc, char *parv[])
{
  ...
  {
    sendto_one(client_p, form_str(client_p,RPL_LISTSTART), me.name, client_p->name);

    for (chptr_list = GlobalChannelList; chptr_list; chptr_list = chptr_list->nextch)
    {
      if (match(parv[2], chptr_list->chname))
      {
        sendto_one(client_p, form_str(client_p,RPL_LIST), me.name, client_p->name,
                   chptr_list->chname, chptr_list->users, chptr_list->topic == NULL ? "" : chptr_list->topic);
      }
    }

    sendto_one(client_p, form_str(client_p,RPL_LISTEND), me.name, client_p->name);
    return;
  }
#endif
  ...
  if (strcasecmp(parv[1], "MODE") == 0)
  {
    ...
    {
      sendto_one(client_p, form_str(client_p,ERR_BADCHANNAME),
    ...
    if ((chptr_mode = (struct Channel *)hash_find_channel(parv[2])) == NULL)
    {
       ...
       * info.
       */ 
      sendto_one(client_p, form_str(client_p,ERR_NOSUCHCHANNEL),
    ...
    sendto_one(client_p, form_str(client_p,RPL_CHANNELMODEIS),
               me.name, parv[0], parv[2], modebuf, parabuf);
    sendto_one(client_p, form_str(client_p,RPL_CREATIONTIME),
  ...
  if (strcasecmp(parv[1], "TOPIC") == 0)
  {
    ...
    {
      sendto_one(client_p, form_str(client_p,ERR_NOSUCHCHANNEL), me.name,
                 parv[0], parv[2]);
      return;
    }

    if (chptr_topic->topic == NULL)
      sendto_one(client_p, form_str(client_p,RPL_NOTOPIC), me.name, parv[0],
                 parv[2]);
    else
    {
      sendto_one(client_p, form_str(client_p,RPL_TOPIC), me.name, parv[0],
                 chptr_topic->chname, chptr_topic->topic);
      sendto_one(client_p, form_str(client_p,RPL_TOPICWHOTIME), me.name,
  ...
  if (strcasecmp(parv[1], "WHO") == 0)
  {
    if (mask != NULL)
    {
      ...
      if (*mask == '\0')
      {
        sendto_one(client_p, form_str(client_p,RPL_ENDOFWHO), me.name, parv[0], "*");
        return;
      }
    }
    else
    ...
    if (IsChannelName(mask))
    {
      ...
      sendto_one(client_p, form_str(client_p,RPL_ENDOFWHO), me.name, parv[0], mask);
      return;
    }
  ...
  if (strcasecmp(parv[1], "WHOIS") == 0)
  {
    ...
    {
      sendto_one(client_p, form_str(client_p,ERR_NOSUCHNICK), me.name, parv[0], parv[2]);
      return;
    }

    a2client_p = (struct Client *)find_server(target_p->user->server);

    sendto_one(client_p, form_str(client_p,RPL_WHOISUSER), me.name,
               client_p->name, target_p->name, target_p->username,
               target_p->host, target_p->info);
    ircsprintf(buf, form_str(client_p,RPL_WHOISCHANNELS), me.name,
    ...
    sendto_one(client_p, form_str(client_p,RPL_WHOISSERVER), me.name,
               client_p->name, target_p->name, target_p->user->server,
               a2client_p ? a2client_p->info : "*Not On This Net*");

    if (IsOper(target_p))
      sendto_one(client_p, form_str(client_p,RPL_WHOISOPERATOR), me.name,
                 client_p->name, target_p->name);

    if (MyConnect(target_p))
      sendto_one(client_p, form_str(client_p,RPL_WHOISIDLE), me.name,
                   ...
                   char *op_flags)
{
  ...
  if(ConfigServerHide.hide_servers)
    {
      sendto_one(source_p, form_str(source_p,RPL_WHOREPLY), me.name, source_p->name,
                 (chname) ? (chname) : "*",
                 target_p->username,
