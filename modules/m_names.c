 ...
 *  USA
 *
 *  $Id: m_names.c,v 1.2 2003/03/10 06:09:01 demond Exp $
 */
...
{ 
  ...
  if (!BadPtr(para))
  ...
  else
    {
      ...
      sendto_one(source_p, form_str(source_p,RPL_ENDOFNAMES), me.name, parv[0],
                 "*");
    }
}
...
static void names_non_public_non_secret(struct Client *source_p)
{
  ...
  ircsprintf(buf,form_str(source_p,RPL_NAMREPLY),
