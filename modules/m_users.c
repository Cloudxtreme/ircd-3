 ...
 *  USA
 *
 *  $Id: m_users.c,v 1.4 2003/05/24 00:07:45 demond Exp $
 */
                   ...
                   int parc, char *parv[])
{
  ...
  sendto_one(source_p, form_str(source_p,RPL_LOCALUSERS), me.name, parv[0],
             ConfigServerHide.hide_servers ? Count.total : Count.local, 
	     ConfigServerHide.hide_servers ? Count.max_tot : Count.max_loc);

  sendto_one(source_p, form_str(source_p,RPL_GLOBALUSERS), me.name, parv[0],
             Count.total, Count.max_tot);

  sendto_one(source_p, form_str(source_p,RPL_IDUSERS), me.name, parv[0],
