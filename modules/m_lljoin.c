 ...
 *  USA
 *
 *  $Id: m_lljoin.c,v 1.2 2003/03/10 06:09:01 demond Exp $
 */
                     ...
                     char *parv[])
{
...
#endif
  {
    ...
    {
      sendto_one(target_p, form_str(target_p,ERR_UNAVAILRESOURCE),
    ...
    {
      sendto_one(target_p,
                 form_str(target_p,i), me.name, nick, root_vchan->chname);
      return;
    }
  }
  ...
  if ((target_p->user->joined >= ConfigChannel.max_chans_per_user) &&
    ...
    {
      sendto_one(target_p, form_str(target_p,ERR_TOOMANYCHANNELS),
		 me.name, nick, root_vchan->chname );
      return; 
    }
  ...
  if (PaceChannel(chptr) && (chptr->joins < MAX_PACEJOIN))
    add_pace(chptr, target_p, root_vchan->chname, 1, 1);
  else
