 ...
 *  USA
 *
 *  $Id: m_challenge.c,v 1.2 2003/03/10 06:09:00 demond Exp $
 */
                        ...
                        int parc, char *parv[] )
{
  ...
  if(IsOper(source_p))
  {
    sendto_one(source_p, form_str(source_p,RPL_YOUREOPER), me.name, parv[0]);
  ...
  if (*parv[1] == '+')
  {
    ...
    {
      sendto_one(source_p, form_str(source_p,ERR_PASSWDMISMATCH), me.name,
		 source_p->name);
      return;
    }
    ...
    {
      sendto_one (source_p, form_str(source_p,ERR_NOOPERHOST), me.name, parv[0]);
  ...
  {
    sendto_one (source_p, form_str(source_p,RPL_RSACHALLENGE), me.name, parv[0],
		challenge);
  }
