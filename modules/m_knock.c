 ...
 *  USA
 *
 *  $Id: m_knock.c,v 1.3 2003/05/24 00:07:45 demond Exp $
 */
	...
	char *parv[])
{
  ...
  if((ConfigChannel.use_knock == 0) && MyClient(source_p))
    {
      sendto_one(source_p, form_str(source_p,ERR_KNOCKDISABLED),
                              ...
                              int parc, char *parv[], char *sockhost)
{
  ...
  if(!(chptr = hash_find_channel(name)))
  {
    ...
    {
      sendto_one(source_p, form_str(source_p,ERR_NOSUCHCHANNEL), me.name, parv[0],
                 name);
    }
  ...
  if (IsVchanTop(chptr))
  {
    ...
    {
      sendto_one(source_p, form_str(source_p,ERR_KNOCKONCHAN),
  ...
  else if (IsVchan(chptr))
  {
    ...
    sendto_one(source_p, form_str(source_p,ERR_BADCHANNAME), me.name, parv[0],
	       name);
    return;
  }
  else
  ...
  {
    sendto_one(source_p, form_str(source_p,ERR_CHANOPEN),
	       me.name, source_p->name, name);
    return;
  }
  ...
  {
    sendto_one(source_p, form_str(source_p,ERR_CANNOTSENDTOCHAN), me.name, parv[0],
	       name);
    return;
  }
  ...
  {
    sendto_one(source_p, form_str(source_p,ERR_TOOMANYKNOCK),
	   ...
	   struct Channel *chptr, char *name, char *key, int llclient)
{
  ...
  if(MyClient(source_p))
  {
    ...
    sendto_one(source_p, form_str(source_p,RPL_KNOCKDLVR),
               me.name, source_p->name, name);
  }
  else if(llclient == 1)
    sendto_one(source_p, form_str(source_p,RPL_KNOCKDLVR),
