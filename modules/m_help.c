 ...
 *  USA
 *
 *  $Id: m_help.c,v 1.2 2003/03/10 06:09:01 demond Exp $
 */
                   ...
                   char *topic, char *nick)
{
  ...
  if (strchr(topic, '/'))
  {
    sendto_one(source_p, form_str(source_p,ERR_HELPNOTFOUND), me.name, nick, topic);
    return;
  }
                         ...
                         char *topic, char *nick)
{
  ...
  sendto_one(source_p, form_str(source_p,RPL_HELPSTART), me.name, nick, topic, line);

  while (fbgets(line, sizeof(line), file))
  {
    sendto_one(source_p, form_str(source_p,RPL_HELPTXT), me.name, nick, topic, line);
  }

  sendto_one(source_p, form_str(source_p,RPL_ENDOFHELP), me.name, nick, topic);
  return;
}
