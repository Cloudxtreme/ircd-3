 ...
 *  USA
 *
 *  $Id: m_topic.c,v 1.4 2003/05/24 00:07:45 demond Exp $
 */
      ...
      if (parc > 2)
	{
	  ...
	  if ((chptr->mode.mode & MODE_TOPICLIMIT) == 0 ||
	      is_any_op(chptr,source_p))
	    {
	      ...
	      if (NoColorChannel(chptr)) 
			/*strip_color(parv[2]);*/
			parv[2] = strip_color(parv[2], 0);
	  ...
	  if (!IsMember(source_p, chptr) && SecretChannel(chptr))
	    {
          ...
          if (chptr->topic == NULL)
	    sendto_one(source_p, form_str(source_p,RPL_NOTOPIC),
		       me.name, parv[0], parv[1]);
          else
	    {
	          char foo[TOPICLEN]; strlcpy(foo, chptr->topic, TOPICLEN);
              sendto_one(source_p, form_str(source_p,RPL_TOPIC),
                         me.name, parv[0],
                         root_chan->chname, 
  ...
  if (parv[1] && IsChannelName(parv[1]))
    {
      ...
      /*if ((time_t)atoi(parv[3]) > chptr->topic_time)
        return;*/
