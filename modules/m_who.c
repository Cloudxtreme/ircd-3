 ...
 *  USA
 *
 *  $Id: m_who.c,v 1.2 2003/03/10 06:09:02 demond Exp $
 */
#include "stdinc.h"
#include "tools.h"
#include "common.h"   
                 ...
                 char *parv[])
{
  ...
  if (mask != NULL)
    {
      ...
      if (*mask == '\0')
	{
	  sendto_one(source_p, form_str(source_p,RPL_ENDOFWHO), me.name, parv[0], "*");
	  return;
	}
    }
  else
  ...
  if (IsChannelName(mask))
      ...
      if (chptr != NULL)
	{
      ...
      sendto_one(source_p, form_str(source_p,RPL_ENDOFWHO), me.name, parv[0], mask);
      return;
    }
