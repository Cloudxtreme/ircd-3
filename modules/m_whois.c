 ...
 *  USA
 *
 *  $Id: m_whois.c,v 1.4 2003/05/24 00:07:45 demond Exp $
 */
	...
	int parc, char *parv[])
{
  ...
  if(parc > 2)
  {
	...
	sendto_one(source_p,form_str(source_p,RPL_LOAD2HI),me.name,source_p->name);
      return;
    }
    else
	 ...
	 int parc, char *parv[])
{
  ...
  if(found)
    sendto_one(source_p, form_str(source_p,RPL_ENDOFWHOIS), me.name, parv[0], parv[1]);
  else
    sendto_one(source_p, form_str(source_p,ERR_NOSUCHNICK), me.name, parv[0], nick);
	     ...
	     int wilds, int glob)
{
  ...
  {
    sendto_one(source_p, form_str(source_p,RPL_WHOISUSER), me.name,
	       source_p->name, name,
	       target_p->username, target_p->host, target_p->info);
    sendto_one(source_p, form_str(source_p,RPL_WHOISSERVER),
...
whois_person(struct Client *source_p,struct Client *target_p, int glob)
{
  ...
  if (IsOper(source_p) || (target_p == source_p))
    sendto_one(source_p, form_str(source_p,RPL_WHOISREALHOST), me.name,
	       source_p->name, target_p->name,
	       ServerInfo.network_name, target_p->realhost);

  if (target_p->svsflags && find_services()) 
    {
      t = buf; t[0] = 0;
      
      if (target_p->svsflags & FLAGS_SVS_IDENT)
        { ircsprintf(t, "this nick"); t += strlen(t); }

      if (target_p->svsflags & FLAGS_SVS_IDENT &&
          target_p->svsflags & FLAGS_SVS_ADMIN)
        { ircsprintf(t, ", "); t += strlen(t); }

      if (target_p->svsflags & FLAGS_SVS_ADMIN)
        { ircsprintf(t, "services admin"); t += strlen(t); }

      sendto_one(source_p, form_str(source_p,RPL_WHOISSVSHINT), me.name,
               source_p->name, target_p->name, buf);
    }

  if (find_z_conf(server_name) && !IsOper(source_p))
    goto rplserver;

  ircsprintf(buf, form_str(source_p,RPL_WHOISCHANNELS),
 ...
 rplserver:          
  ...
  if (IsOper(target_p))
  	{
    	sendto_one(source_p, form_str(source_p,RPL_WHOISOPERATOR),
	    	me.name, source_p->name, target_p->name);

      	if (IsAdmin(target_p))
        	sendto_one(source_p, form_str(source_p,RPL_WHOISADMIN),
            	me.name, source_p->name, target_p->name);
	}
  ...
  if (MyClient(target_p)) {
  	...
  	if (F && F->ssl)
  		sendto_one(source_p, form_str(source_p,RPL_WHOISSSL),
  			me.name, source_p->name, target_p->name);	
  }
#endif
  ...
  if (ConfigFileEntry.cyrillic)
    {
      char *p;
      for (p = target_p->name; *p; p++)
        {
          if ((unsigned char)*p >= 192)
            {
              sendto_one(source_p, form_str(source_p,RPL_WHOISCYRILLIC),
                         me.name, source_p->name, target_p->name);
              break;
            }
        }
  ...
  {
    sendto_one(source_p, form_str(source_p,RPL_WHOISIDLE),
	       me.name, source_p->name, target_p->name,
