 ...
 *  USA
 *
 *  $Id: m_stats.c,v 1.3 2003/05/24 00:07:45 demond Exp $
 */
...
static void stats_userparms(struct Client *);
static void stats_class(struct Client *);
static void stats_memory(struct Client *);
static void stats_services(struct Client *);
static void stats_equivs(struct Client *);
static void stats_relays(struct Client *);
...
static void stats_clusters(struct Client *);
                   ...
                   int parc, char *parv[])
{
  ...
  for (i=0; stats_cmd_table[i].handler; i++)
  {
    if (stats_cmd_table[i].letter == statchar)
    {
      ...
      {
        sendto_one(source_p, form_str(source_p,ERR_NOPRIVILEGES),me.name,source_p->name);
        break;
      }
  ...
  sendto_one(source_p, form_str(source_p,RPL_ENDOFSTATS), me.name, parv[0], parv[1]);
...
static void stats_deny(struct Client *source_p)
{
  ...
  for (i=0; i < ATABLE_SIZE; i++)
  {
	...
	sendto_one(source_p, form_str(source_p,RPL_STATSDLINE), me.name,
...
stats_glines(struct Client *source_p)
{
  ...
  DLINK_FOREACH(gline_node, glines.head)
  {
    ...
    sendto_one(source_p, form_str(source_p,RPL_STATSKLINE), me.name,
...
stats_auth(struct Client *source_p)
{
  ...
  else if((ConfigFileEntry.stats_i_oper_only == 1) && !IsOper(source_p))
  {
    ...
    sendto_one(source_p, form_str(source_p,RPL_STATSILINE), me.name,
...
static void stats_services(struct Client *client_p)
{
  report_zlines(client_p);
}

static void stats_equivs(struct Client *client_p)
{
  report_equivs(client_p);
}

static void stats_relays(struct Client *client_p)
{
  report_relays(client_p);
}

static void stats_userparms(struct Client *client_p)
{
  report_userparms(client_p);
}

static void stats_clusters(struct Client *client_p)
{
  report_clusters(client_p);
}
...
stats_uptime(struct Client *source_p)
{
  time_t now;
   ...
   sendto_one(source_p, form_str(source_p,RPL_STATSUPTIME), me.name, source_p->name,
