 ...
 *  USA
 *
 *  $Id: m_sjoin.c,v 1.3 2003/05/24 00:07:46 demond Exp $
 */
                    ...
                    char *parv[])
{
  ...
  *sjbuf_nhops = '\0';
  ...
  cluster_prefix(client_p, parc, parv, parc - 1, 0, 1);
  ...
  while (*s)
	...
	if(ConfigChannel.use_anonops)
      ...
      case 'x':
        ...
        break;
      case 'z':
        mode.mode |= MODE_NOREPEAT;
        break;
      case 'R':
        mode.mode |= MODE_REGNICKS;
        break;
      case 'C':
        mode.mode |= MODE_CYRNICKS;
        break;
      case 'M':
        mode.mode |= MODE_REGSPEAK;
        break;
      case 'S':
	  ...
	  if (top_chptr)
	    {
          ...
          if (PaceChannel(chptr) && (chptr->joins < MAX_PACEJOIN))
	        add_pace(chptr, target_p, top_chptr->chname, 1, 0);
	      else
	    ...
	    {
          if (PaceChannel(chptr) && (chptr->joins < MAX_PACEJOIN))
	        add_pace(chptr, target_p, parv[2], 1, 0);
	      else
	      	sendto_channel_local(ALL_MEMBERS,chptr, ":%s!%s@%s JOIN :%s",
				   target_p->name,
				   target_p->username,
