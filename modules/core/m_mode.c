 ...
 *  USA
 *
 *  $Id: m_mode.c,v 1.3 2003/05/24 00:07:46 demond Exp $
 */
              ...
              int parc, char *parv[])
{
  ...
  if (IsChannelName(parv[1]))
  	cluster_prefix(client_p, parc, parv, 2, 0, 2);

/*  if (client_p->user)
	  client_p->user->known = 1;

  this marks drones as known clients... */
  ...
  if ((parc > 2) && parv[2][0] == '!')
    {
     ...
     if (!(target_p = find_client(++parv[2])))
       {
        sendto_one(source_p, form_str(source_p,ERR_NOSUCHCHANNEL), me.name,
                   parv[0], root->chname);
        return;
       }
  ...
  if (parc < n+1)
    {
      ...
      sendto_one(source_p, form_str(source_p,RPL_CHANNELMODEIS),
		 me.name, parv[0], parv[1],
		 modebuf, parabuf);
       ...
       */
      if (!IsOper(source_p))
	sendto_one(source_p, form_str(source_p,RPL_CREATIONTIME),
		   me.name, parv[0],
