 ...
 *  USA
 *
 *  $Id: m_nick.c,v 1.4 2003/05/24 00:07:46 demond Exp $
 */
        ...
        int parc, char *parv[])
{
  ...
  {
    sendto_one(source_p, form_str(source_p,ERR_NONICKNAMEGIVEN),
  ...
  {
    sendto_one(source_p, form_str(source_p,ERR_ERRONEUSNICKNAME),
  ...
  {
    sendto_one(source_p, form_str(source_p,ERR_UNAVAILRESOURCE),
  ...
  if ((target_p = find_client(nick)) == NULL)
  {
	...
	if(!strcmp(nick, uclient_p->llname))
	{
	  ...
	  sendto_one(source_p, form_str(source_p,ERR_NICKNAMEINUSE), me.name, "*", nick);
	  return;
	}
      }
                   ...
                   int parc, char *parv[])
{
  ...
  if ((target_p = find_client(nick)))
  {
    ...
    {
      sendto_one(source_p, form_str(source_p,ERR_NICKNAMEINUSE), me.name,
                 parv[0], nick);
      return;
    }
 ...
 *    parv[0] = sender prefix
 *    parv[1] = nickname
 *    parv[2] = hop count (plus shifted svsflags when CAP_SVS-able)
 *    parv[3] = TS
 *    parv[4] = umode
 *    parv[5] = username
 *    parv[6] = hostname
 *    parv[7] = server
 *    parv[8] = real hostname (when CAP_HOSTMASK-able)
 *    parv[9] = IP (when CAP_IP-able)
 *    parv[10] = ircname
 */
static void
	...
	int parc, char *parv[])
{
  ...
  if(parc < 2 || BadPtr(parv[1]))
  {
    sendto_one(source_p, form_str(source_p,ERR_NONICKNAMEGIVEN), me.name, parv[0]);
    return;
  }

  /* parc == 3 on nickchange, parc == 9 on new nick */
  if((IsClient(source_p) && (parc != 3)) || 
  	(IsServer(source_p) && ((parc < 9) || (parc > 11))))
  {
    char tbuf[BUFSIZE] = { 0 };
    int j;
  ...
  if (parc >= 9)
    {
      if (check_clean_user(client_p, nick, parv[5], parv[7]) ||
          check_clean_host(client_p, nick, parv[6], parv[7]) || (parc > 9?
          check_clean_host(client_p, nick, parv[8], parv[7]) : 0))
        return;

      /* check the length of the clients gecos */
      if(strlen((parc > 9)? ((parc > 10)? parv[10] : parv[9]) : parv[8]) > REALLEN)
        {
          sendto_realops_flags(FLAGS_ALL, L_ALL, "Long realname from server %s for %s",
                         parv[7], parv[1]);
          parv[(parc > 9)? ((parc > 10)? 10 : 9) : 8][REALLEN] = '\0';
        }
	  ...
	  int parc, char *parv[])
{
  ...
  id = parv[8];
  name = (parc > 10)? ((parc > 11)? parv[11] : parv[10]) : parv[9];
  ...
  if(check_clean_nick(client_p, source_p, nick, parv[1], parv[7]) ||
     ...
     check_clean_host(client_p, nick, parv[6], parv[7]) || (parc > 10?
     check_clean_host(client_p, nick, parv[9], parv[7]) : 0))
    return;
                            ...
                            char *nick, char *newnick, char *server)
{
  ...
  if(!clean_nick_name(nick, 1) || strcmp(nick, newnick))
  {
    ServerStats->is_kill++;
		 ...
		 char *host, char *server)
{
  ...
  if(!clean_host_name(host) && strcmp(host, "<none>"))
    sendto_realops_flags(FLAGS_DEBUG, L_ALL,
 ...
 */
static int
clean_nick_name(char *nick, int fromserver)
{
  char* ch   = nick;
  ...
  for ( ; ch < endp && *ch; ++ch)
    {
      if (fromserver && (ch == nick))
      	continue;
      	
      if (ConfigFileEntry.cyrillic && ((unsigned char)*ch >= 192))
        {
          if (lat)
            break;      /* do not allow mixed lat/cyr nicks */
                 ...
                 char *parv[], time_t newts,char *nick)
{
  ...
  if(IsServer(source_p))
  {
    ...
    if(parc > 2)
	{
      ...
      {
        unsigned int val = atoi(parv[2]);

        source_p->svsflags = val >> 8;
        source_p->hopcount = val & 0xFF;       
      } 
        else source_p->hopcount = atoi(parv[2]);
	}
    if(newts)
    ...
    if (parc > 8)
    {
	 ...
	 m++;
      ...
      return do_remote_user(nick, client_p, source_p, parv[5], parv[6],
			  parv[7], (parc > 9)? ((parc > 10)? parv[10] : parv[9]) : parv[8], 
				(parc > 9)? parv[8] : NULL, (parc > 10)? parv[9] : NULL, NULL);
    }
  }
  ...
  source_p->svsflags &= ~FLAGS_SVS_IDENT;
  return 0;
}
                   ...
                   char *parv[], time_t newts,char *nick)
{
  ...
  return do_remote_user(nick, client_p, source_p, parv[5], parv[6],
			parv[7], name, (parc > 10)? parv[9] : NULL, 
			(parc > 11)? parv[10] : NULL, id);
}			
  ...
  if(IsServer(source_p))
  {
       ...
       (newts == target_p->tsinfo))
    {
      ...
      kill_client_ll_serv_butone(NULL, target_p,
				 ...
				 me.name);
      ServerStats->is_kill++;
      sendto_one(target_p, form_str(target_p,ERR_NICKCOLLISION),
