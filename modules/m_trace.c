 ...
 *  USA
 *
 *  $Id: m_trace.c,v 1.3 2003/05/24 00:07:45 demond Exp $
 */
                    ...
                    int parc, char *parv[])
{
  ...
  if (parc > 1)
    ...
    tname = me.name;
  sendto_one(source_p, form_str(source_p,RPL_ENDOFTRACE), me.name, parv[0], tname);
}
                    ...
                    int parc, char *parv[])
{
    ...
    case HUNTED_PASS: /* note: gets here only if parv[1] exists */
      {
        ...
        if (ac2ptr)
          sendto_one(source_p, form_str(source_p,RPL_TRACELINK), me.name, looking_for,
  ...
  if(!IsOper(source_p) || !dow) /* non-oper traces must be full nicks */
                              /* lets also do this for opers tracing nicks */
    {
      ...
      if(target_p && IsPerson(target_p)) 
      {
        ...
        if (IsOper(target_p))
        {
          sendto_one(source_p, form_str(source_p,RPL_TRACEOPERATOR),
        ...
        {
          sendto_one(source_p,form_str(source_p,RPL_TRACEUSER),
                     me.name, parv[0], class_name, name, 
                     MyOper(source_p) ? ipaddr : 
		     (IsIPSpoof(target_p) ? "255.255.255.255" : ipaddr),
                     CurrentTime - target_p->lasttime,
      ...
      sendto_one(source_p, form_str(source_p,RPL_ENDOFTRACE),me.name,
                 parv[0], tname);
      return;
    }
  ...
  if (!SendWallops(source_p) || !cnt)
    {
       ...
       */
      if(!cnt)
	sendto_one(source_p, form_str(source_p,RPL_TRACESERVER),
  ...
  for (cltmp = ClassList; doall && cltmp; cltmp = cltmp->next)
    if (Links(cltmp) > 0)
      sendto_one(source_p, form_str(source_p,RPL_TRACECLASS), me.name,
		   ...
		   int dow, int link_u_p, int link_s_p)
{
  ...
  switch(target_p->status)
    ...
    case STAT_CONNECTING:
      sendto_one(source_p, form_str(source_p,RPL_TRACECONNECTING), me.name,
      ...
      break;
    case STAT_HANDSHAKE:
      sendto_one(source_p, form_str(source_p,RPL_TRACEHANDSHAKE), me.name,
                 source_p->name, class_name, 
                 IsOperAdmin(source_p) ? name : target_p->name);
    ...
    case STAT_UNKNOWN:
      /* added time -Taner */
      sendto_one(source_p, form_str(source_p,RPL_TRACEUNKNOWN),
    ...
    default: /* ...we actually shouldn't come here... --msa */
      sendto_one(source_p, form_str(source_p,RPL_TRACENEWTYPE), me.name,
		 source_p->name, name);
      cnt++;
      break;
    }
