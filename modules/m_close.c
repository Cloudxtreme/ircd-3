 ...
 *  USA
 *
 *  $Id: m_close.c,v 1.2 2003/03/10 06:09:01 demond Exp $
 */
	 ...
	 int parc, char *parv[])
{
  ...
  /* Which list would connecting servers be found in? serv_list ? */
      ...
      if (!IsUnknown(target_p) && !IsConnecting(target_p) &&
        ...
        continue;
#endif
      sendto_one(source_p, form_str(source_p,RPL_CLOSING), me.name, parv[0],
                 get_client_name(target_p, SHOW_IP), target_p->status);
      (void)exit_client(target_p, target_p, target_p, "Oper Closing");
      closed++;
    }
  sendto_one(source_p, form_str(source_p,RPL_CLOSEEND), me.name, parv[0], closed);
}
