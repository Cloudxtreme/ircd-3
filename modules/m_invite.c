 ...
 *  USA
 *
 *  $Id: m_invite.c,v 1.3 2003/05/24 00:07:45 demond Exp $
 */
         ...
         struct Client *source_p, int parc, char *parv[])
{
  ...
  cluster_prefix(client_p, parc, parv, 2, 1, 0);
  ...
  {
    sendto_one(source_p, form_str(source_p,ERR_NOSUCHNICK), me.name, parv[0], parv[1]);
    return;
  }
  ...
  if (!MyConnect(target_p) && (parv[2][0] == '&'))
  {
    if (ConfigServerHide.hide_servers == 0)
      sendto_one(source_p, form_str(source_p,ERR_USERNOTONSERV),
  ...
  {
    sendto_one(source_p, form_str(source_p,ERR_NOTONCHANNEL), me.name, parv[0],
               parv[2]);
    return;
  }
  ...
  if ((vchan2 = map_vchan(chptr, target_p)))
  {
    if (MyClient(source_p) && ((vchan2->mode.mode & MODE_SECRET) == 0))
      sendto_one(source_p, form_str(source_p,ERR_USERONCHANNEL), me.name, parv[0],
                 parv[1], parv[2]);
    return;
  }
#endif
  ...
  {
    if (MyClient(source_p))
      sendto_one(source_p, form_str(source_p,ERR_USERONCHANNEL),
  ...
  if (chptr && (vchan->mode.mode & MODE_INVITEONLY))
  {
    ...
    {
      if (MyClient(source_p))
        sendto_one(source_p, form_str(source_p,ERR_CHANOPRIVSNEEDED),
                   me.name, parv[0], parv[2]);
      return;
    }
  }
  ...
  {
    sendto_one(source_p, form_str(source_p,RPL_INVITING), me.name, parv[0],
               target_p->name, parv[2]);
    if (target_p->user->away)
      sendto_one(source_p, form_str(source_p,RPL_AWAY), me.name, parv[0],
