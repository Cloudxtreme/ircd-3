 ...
 *  USA
 *
 *  $Id: m_whowas.c,v 1.3 2003/03/10 06:09:02 demond Exp $
 */
                     ...
                     int parc, char *parv[])
{
  ...
  for(;temp;temp=temp->next)
    {
      if (!irccmp(nick, temp->name))
        {
          sendto_one(source_p, form_str(source_p,RPL_WHOWASUSER),
          ...
          if (IsOper(source_p))
    		sendto_one(source_p, form_str(source_p,RPL_WHOISREALHOST), 
    			me.name, parv[0], temp->name,
  ...
  if (!found)
    sendto_one(source_p, form_str(source_p,ERR_WASNOSUCHNICK),
               me.name, parv[0], nick);

  sendto_one(source_p, form_str(source_p,RPL_ENDOFWHOWAS), me.name, parv[0], parv[1]);
  return 0;
}
