 ...
 *  USA
 *
 *  $Id: channel_mode.h,v 1.2 2003/03/10 06:09:00 demond Exp $
 */
...
#define CAN_SEND_NOTREG 3      /* nonreg nicks cannot send to +M chans */
...
#define CHFL_SUBST		0x0040 /* word subst flag */
#define CHFL_INVEX      0x0080
#define CHFL_DENY       0x0100 /* gecos & channame deny flag */
...
#define MODE_REGCHAN    0x0001  /* channel is registered with services */
#define MODE_REGNICKS   0x0002  /* only registered nicknames can join  */
#define MODE_REGSPEAK	0x0004  /* only registered nicknames may speak */
...
#define MODE_NOCOLOR    0x0200  /* strip mIRC & ANSI color */
#define MODE_BAN        0x0400
#define MODE_EXCEPTION  0x0800
#define MODE_PACE		0x1000  /* pacing JOIN, PART, PRIVMSG and QUIT */
#define MODE_INVEX		0x2000
#define MODE_HIDEOPS    0x4000
#define MODE_CYRNICKS	0x8000  /* cyrillic nicks are NOT allowed to join */
#define MODE_NOREPEAT  0x10000  /* no public repeating allowed */
...
/* colors stripped from chanmsgs, notices, ctcps, quits */
#define NoColorChannel(x)       ((x) && ((x)->mode.mode & MODE_NOCOLOR))
#define RegNicksOnlyChannel(x)  ((x) && ((x)->mode.mode & MODE_REGNICKS))
#define RegsMaySpeakChannel(x)  ((x) && ((x)->mode.mode & MODE_REGSPEAK))
#define NoCyrNicksChannel(x)    ((x) && ((x)->mode.mode & MODE_CYRNICKS))
/* we are pacing JOINs, PARTs, PRIVMSGs and QUITs sent to the channel */
#define PaceChannel(x)			((x) && ((x)->mode.mode & MODE_PACE))
#define NoRepeatChannel(x)		((x) && ((x)->mode.mode & MODE_NOREPEAT))
