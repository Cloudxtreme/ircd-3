 ...
 *  USA
 *
 *  $Id: client.h,v 1.2 2003/03/10 06:09:00 demond Exp $
 */
...
struct User
{
  ...
  dlink_list	 repeat;		/* chain of last phrases per channel blocks */
  ...
  int            known;			/* USER came from particular type of client */
};
...
struct Client
{
  ...
  unsigned int      svsflags;   /* services flags */
  ...
  /*
   * this contains client's real host, whereas host[] field
   * is modified (in register_local_user) to store masked hostname
   *                                                     -demond
   */
  char          	realhost[HOSTLEN + 1];
  /*
   * this contains client's IP, sent by CAP_IP-able server
   *                                                     -demond
   */
  char          	ip[HOSTLEN + 1];
  /*
...
#define FLAGS_REMOTE       0x2000 /* Remote client registering */
...
#define FLAGS_TRANSWIN     0x100000 /* translate cyrillic Windows-1251 */
#define FLAGS_TRANSISO     0x200000 /* translate cyrillic ISO-8859-5 */
#define FLAGS_REGACCEPT    0x400000 /* accepting from reg nicks only */
#define FLAGS_ALTLOCALE    0x800000 /* receiving numeric replies in alt textdomain */
...
#define FLAGS_SVS_IDENT    0x0001  /* client has identified with services */
#define FLAGS_SVS_ADMIN    0x0002  /* client has identified as services admin */
...
#define SEND_UMODES  (FLAGS_INVISIBLE | FLAGS_OPER | FLAGS_WALLOP | \
                      FLAGS_ADMIN | FLAGS_ALTLOCALE | FLAGS_SSLACCEPT)
#define ALL_UMODES   (SEND_UMODES | FLAGS_SERVNOTICE | FLAGS_CCONN | \
                      ...
                      FLAGS_BOTS | FLAGS_EXTERNAL | FLAGS_LOCOPS | \
 		      		  FLAGS_REMOTE | FLAGS_UNAUTH | FLAGS_CALLERID | \
					  FLAGS_TRANSWIN | FLAGS_TRANSISO | FLAGS_REGACCEPT)
...
#define SendRemoteNotice(x)		((x)->umodes & FLAGS_REMOTE)
#define SetTransWin(x)          ((x)->umodes |= FLAGS_TRANSWIN)
#define IsSetTransWin(x)        ((x)->umodes & FLAGS_TRANSWIN)
#define SetTransISO(x)          ((x)->umodes |= FLAGS_TRANSISO)
#define IsSetTransISO(x)        ((x)->umodes & FLAGS_TRANSISO)
#define SetRegAccept(x)         ((x)->umodes |= FLAGS_REGACCEPT)
#define IsSetRegAccept(x)       ((x)->umodes & FLAGS_REGACCEPT)
#define SetSSLaccept(x)         ((x)->umodes |= FLAGS_SSLACCEPT)
#define IsSetSSLaccept(x)       ((x)->umodes & FLAGS_SSLACCEPT)
#define SetAltLocale(x)         ((x)->umodes |= FLAGS_ALTLOCALE)
#define IsSetAltLocale(x)       ((x)->umodes & FLAGS_ALTLOCALE)
...
extern char *translate(struct Client *client_p, char *buf);
